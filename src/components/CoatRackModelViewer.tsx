...
import "./CoatRackModelViewer.css";

type CoatRackModelViewerProps = {
  open: boolean;
  onClose: () => void;
};

function CoatRackScene() {
  ...
  useEffect(() => {
    ...
    const calibratedDefaultCamera = new THREE.Vector3(-3.6588, 3.4198, 12.8399);
    const calibratedDefaultTarget = new THREE.Vector3(0, 2.4, 0);
    const calibratedDefaultDistance = 13.3899;
    // Calibrated main/front view supplied for the third view button.
    const calibratedFrontCamera = new THREE.Vector3(2.7041, 2.8558, 13.2973);
    const calibratedFrontTarget = new THREE.Vector3(1.2, 2.4, 0);
    ...
    const attachModel = (model: THREE.Object3D) => {
      ...
      try {
        ...
        console.error("Failed to parse coat-rack model", error);
    ...
    const loadModel = async () => {
      try {
        ...
        const compressedUrl = preview
          ? "/models/coat-rack-preview.fbx.gz.bin"
          : "/models/coat-rack.fbx.gz.bin";
        ...
        console.warn("Compressed coat-rack model unavailable; falling back to FBX", compressedError);
        const loader = new FBXLoader();
        loader.load("/models/coat-rack.fbx", attachModel, (event) => {
          if (!event.total) return;
          setProgress(Math.min(99, Math.round((event.loaded / event.total) * 100)));
        }, (error) => {
          console.error("Failed to load coat-rack model", error);
          if (!disposed) setFailed(true);
        });
      }
    };
  ...
  return (
    <div className={`forklift-scene coat-rack-scene${ready ? " is-ready" : ""}`}>
      <div className="forklift-contact-shadow" aria-hidden="true" />
      <canvas ref={canvasRef} aria-label="可拖动旋转和缩放的衣帽架三维模型" />
      {!ready && !failed && (
...
export default function CoatRackModelViewer({ open, onClose }: CoatRackModelViewerProps) {
  useEffect(() => {
    if (!open) return;
  ...
  if (!open) return null;
  return (
    <div className="model-viewer-backdrop" role="dialog" aria-modal="true" aria-label="FOLD 衣帽架设计展示页">
      <section className="model-viewer-panel coat-rack-model-viewer-panel">
        <img className="model-viewer-reference" src="/coat-rack-model-page.jpg" alt="FOLD 衣帽架设计展示页" draggable={false} />
        <CoatRackScene />
        <button className="model-viewer-close-hotspot" type="button" onClick={onClose} aria-label="关闭衣帽架设计展示页"><span aria-hidden="true" /></button>
      </section>
    </div>
  );
}
