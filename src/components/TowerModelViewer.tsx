...
function TowerScene() {
  ...
  useEffect(() => {
    ...
    const calibratedDefaultCamera = new THREE.Vector3(-0.8448, 2.7952, 12.6577);
    const calibratedDefaultTarget = new THREE.Vector3(0, 2.496, 0);
    const calibratedDefaultDistance = 12.6893;
    // Exact main/front view calibration supplied for the third view button.
    const calibratedFrontCamera = new THREE.Vector3(-5.2365, 2.9744, 11.5485);
    ...
    const attachModel = (model: THREE.Object3D) => {
      ...
      try {
        ...
        console.error("Failed to parse tower fan model", error);
    ...
    const loadModel = async () => {
      try {
        ...
        const compressedUrl = preview
          ? "/models/tower-fan-preview.fbx.gz.bin"
          : "/models/tower-fan.fbx.gz.bin";
        ...
        console.warn("Compressed tower fan model unavailable; falling back to FBX", compressedError);
        const loader = new FBXLoader();
        loader.load("/models/tower-fan.fbx", attachModel, (event) => {
          if (!event.total) return;
          setProgress(Math.min(99, Math.round((event.loaded / event.total) * 100)));
        }, (error) => {
          console.error("Failed to load tower fan model", error);
          if (!disposed) setFailed(true);
        });
      }
    };
  ...
  return (
    <div className={`forklift-scene tower-scene${ready ? " is-ready" : ""}`}>
      <div className="forklift-contact-shadow" aria-hidden="true" />
      <canvas ref={canvasRef} aria-label="可拖动旋转和缩放的塔扇三维模型" />
      {!ready && !failed && (
...
export default function TowerModelViewer({ open, onClose }: TowerModelViewerProps) {
  useEffect(() => {
    if (!open) return;
  ...
  if (!open) return null;
  return (
    <div className="model-viewer-backdrop" role="dialog" aria-modal="true" aria-label="德信塔扇设计展示页">
      <section className="model-viewer-panel tower-model-viewer-panel">
        <img className="model-viewer-reference" src="/tower-fan-model-page.jpg" alt="德信塔扇设计模型展示页" draggable={false} />
        <TowerScene />
        <button className="model-viewer-close-hotspot" type="button" onClick={onClose} aria-label="关闭塔扇设计展示页"><span aria-hidden="true" /></button>
      </section>
    </div>
  );
}
